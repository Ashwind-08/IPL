...
/***********Add Team and Player in page ---code  starts Here**************/
var addteamclicked = () => {
...
/************** Add Team and Player in page --code ends Here*/


/************* Teams Container Strats Here**********/
let detailofTeam = [];
let detailOfPlayer = [];
...
/* This will helps to listen the keyboard keys press when 
the user press the the ENTER key KEYCODE of Enter is 13 */
inputBox.onkeyup = (e) => {
  if (e.keyCode == 13) {
    icon.click();
  }
  let userData = e.target.value;//this will returns a how many players in the Team
  console.log(userData)
  let emptyArray = [];//empty array for Suggestion



 /*  Filtering the  data from the suggestArray to comparing the 
  user enter data and the available data in the local Storage and converting the
   entire typed data into lowerCase form */
  if (userData) {
    ...
    for (let i = 0; i < allList.length; i++) {
      allList[i].setAttribute("onclick", "currentLi(this)");//this will specify the  current object li 
...
//when you clicking that suggestion it will put that suggestion into input.textContent
function currentLi(element) {
  let selectData = element.textContent;
  inputBox.value = selectData;
  //if i click on that search icon it will redirect to teams.html page
  icon.onclick = () => {
    window.open(`./teams.html?name=${element.textContent}`, "_self");
  };
  searchBar.classList.remove("active");
}
/* // It takes the input from the user which ever they are typing in and
 put it into the input.textContent */

function showSuggestions(list) {
  let listData;
  if (!list.length) {
    userValue = inputBox.value;//this will be the value of the user typed.
    listData = `<li>${userValue}</li>`;
  } else {
    listData = list.join("");
  }
  suggBox.innerHTML = listData;//this will put into input innerHTML as a text
}
